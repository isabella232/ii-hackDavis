...
const { sendClientToVerifyEmail } = require("../utils/email");
...
router.get("/api/client/home", auth, async (req, res) => {
  try {
    ...
    const now = new Date();
    const events = await Event.find({ isArchived: false, forClients: true })
      .where("date")
      .gte(now);
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      if (req.file) {
        client.avatar.buffer = await sharp(req.file.buffer)
          .resize({ width: 250, height: 250 })
          .png()
          .toBuffer();
      }
...
router.patch("/api/client/unbookmarkInterpreter", auth, async (req, res) => {
  const client = req.user;
  try {
    client.bookmarks = client.bookmarks.filter(
